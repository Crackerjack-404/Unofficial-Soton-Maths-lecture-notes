...
In single-variable calculus, we have seen: 
$$
f: \mathbb{R} \rightarrow \mathbb{R} 
$$

But now in multivariable, as the name suggests, we can have: 
$$
f: \mathbb{R}^n \rightarrow \mathbb{R} , \quad f: \mathbb{R}^m \rightarrow \mathbb{R}^m
$$

Unlike one variable, where points live on a line, the inputs in multivariable calculus live in a space. So we need to understand what space is because we can start talking about limits and other things that we explored in single variable calculus. 


\section{Euclidean Space}

Previously in calculus, when talking about limits in 1D, we only had two directions - namely left or right. 

Now in $\mathbb{R}^2$ or $\mathbb{R}^3$, we can have infinitely many directions. So to deal with these changes, we need to think about distances, open balls, neighbourhoods, and topology. 


\section{Open and Closed sets}

Now instead of talking about in 1D, we can talk about open and closed sets in $\mathbb{R}^n$. 

A set is said to be \textbf{open} if around every point, we can fit a small open ball fully inside the set. And a set is \textbf{closed} if it contains its boundary points. This distinction will become more apparent when we start talking about differentiation.

\section{Neighbourhoods}

Take a single variable, say $x$, then the neighbourhood of $x$ is simply a little interval that it lies in, eg: 
$$
(x-\delta, x+\delta)
$$

Now, in multivariable, say in $\mathbb{R}^2$, a neighbourhood of a point $p$ is $p=(a,b)$ s.t.: 
$$
\set{(x,y) = \sqrt{(x-a)^2 + (y-b)^2} < \delta}
$$

This defines a little disc, or all points within some small radius. 

\section{Explicit and Implicit Functions}

The implicit form of a function describes a sphere from which we get two branches: upper and lower hemisphere. And then applying restrictions lets us pick one branch. 

\chapter{Geometry of Functions}

\section{Conic sections}

Let's start with a general form of a 2nd degree curve: 
$$
Ax^2 + Bxy + Cy^2 + Dx + Ey + F = 0
$$

Depending on coefficients, we can get: 
\begin{itemize}
    \item Ellipse 
    \item Parabola 
    \item Hyperbola
...
The reason is simple, it's what shape you get when you intersect a plane with a quadratic surface, and conics come from slicing surfaces. 

\chapter{Limits}

In 1D, limits are often written as: 
$$
\lim_{x \to a} f(x) = L
$$
which essentially means that when $x$ is close to $a$, $f(x)$ is close to $L$ 

However, now in 2D, we need to think about limits from multiple directions: 
$$
\lim_{(x,y) \to (a,b)} f(x,y) = L 
$$

Similarly, this means that whenever $(x,y)$ is close to $(a,b)$ from ANY direction, the value of $f(x,y)$ is close to $L$

Key takeaway, in 1D, we only have two direction, but in 2D, we have infinitely many paths we can take. 

\section{Formal definition}

Our epsilon-delta definition of a limit also changes here, so we have: 
$$
\forall \epsilon > 0, \exists \delta > 0 \ \ s.t. 0 < \sqrt{(x-a)^2 + (y-b)^2} > \delta \implies | f(x,y)- L| \leq \epsilon
$$

Now instead instead of closed intervals, we're dealing with neighbourhood of $(a,b)$

\section{Continuity and Domain}

A function is continuous at $(a,b)$ if: 
\begin{itemize}
    \item $(a,b)$ is in the domain 
    \item the limit exists 
    \item the limit equals the function value
\end{itemize}

So continuity depends on both the limit and the domain containing interior points. 


\section{(non)-Existence of limits}

In multivariable calc, if two different paths give different limits, then the limit does not exist. So we don't need to check infinitely many points, but only two contradictory paths. 

The methods of finding limits can be found in the notes, so I won't talk about that. 
